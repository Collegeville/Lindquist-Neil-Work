...
void CheckProblem(SparseMatrix<DatatypeA> & A, Vector<DatatypeB> * b, Vector<DatatypeX> * x, Vector<DatatypeX> * xexact) {
  ...
  for (local_int_t iz=0; iz<nz; iz++) {
    ...
    for (local_int_t iy=0; iy<ny; iy++) {
      ...
      for (local_int_t ix=0; ix<nx; ix++) {
        ...
        DatatypeA * currentValuePointer = A.matrixValues[currentLocalRow]; // Pointer to current value in current row
        ...
        if (b!=0)      assert((b->values)[currentLocalRow] == 26.0 - ((DatatypeB) (numberOfNonzerosInRow-1)));
        if (x!=0)      assert((x->values)[currentLocalRow] == 0.0);
        if (xexact!=0) assert((xexact->values)[currentLocalRow] == 1.0);
      } // end ix loop
    } // end iy loop
  } // end iz loop
#ifdef HPCG_DETAILED_DEBUG
