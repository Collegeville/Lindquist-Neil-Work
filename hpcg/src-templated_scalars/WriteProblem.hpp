...
int WriteProblem( const Geometry & geom, const SparseMatrix<DatatypeA> & A,
    const Vector<DatatypeB> b, const Vector<DatatypeX> x, const Vector<DatatypeX> xexact) {
  ...
  for (global_int_t i=0; i< nrow; i++) {
    const DatatypeA * const currentRowValues = A.matrixValues[i];
