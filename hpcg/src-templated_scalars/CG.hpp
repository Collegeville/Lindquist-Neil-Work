...
int CG(const SparseMatrix<Datatype> & A, CGData<Datatype> & data, const Vector<Datatype> & b, Vector<Datatype> & x,
    const int max_iter, const Datatype tolerance, int & niters, Datatype & normr, Datatype & normr0,
    double * times, bool doPreconditioning) {

  double t_begin = mytimer();  // Start timing right away
  normr = 0.0;
  Datatype rtz = 0.0, oldrtz = 0.0, pAp = 0.0;
...
//#endif
  ...
  Vector<Datatype> & r = data.r; // Residual vector
  Vector<Datatype> & z = data.z; // Preconditioned residual vector
  Vector<Datatype> & p = data.p; // Direction vector (in MPI mode ncol>=nrow)
  Vector<Datatype> & Ap = data.Ap;
