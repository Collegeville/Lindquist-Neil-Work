...
int main(int argc, char * argv[]) {
  ...
  Vector<double> b, x, xexact;
  GenerateProblem(A, &b, &x, &xexact);
  SetupHalo(A);
  int numberOfMgLevels = 4; // Number of levels including first
  SparseMatrix<double> * curLevelMatrix = &A;
  ...
  curLevelMatrix = &A;
  Vector<double> * curb = &b;
  Vector<double> * curx = &x;
  Vector<double> * curxexact = &xexact;
  ...
  t1 = mytimer();
#endif
  TestCGData<double> testcg_data;
  testcg_data.count_pass = testcg_data.count_fail = 0;
  TestCG(A, data, b, x, testcg_data);

  TestSymmetryData<double> testsymmetry_data;
...
// Get the absolute worst time across all MPI ranks (time in CG can be different)
  ...
  TestNormsData<double> testnorms_data;
