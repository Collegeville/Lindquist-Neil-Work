...
template<class DatatypeA, class DatatypeB, class DatatypeX>
void GenerateProblem_ref(SparseMatrix<DatatypeA> & A, Vector<DatatypeB> * b, Vector<DatatypeX> * x, Vector<DatatypeX> * xexact) {
  ...
  for (local_int_t i=0; i< localNumberOfRows; ++i)
    matrixValues[i] = new DatatypeA[numberOfNonzerosPerRow];
  for (local_int_t i=0; i< localNumberOfRows; ++i)
  ...
  matrixValues[0] = new DatatypeA[localNumberOfRows * numberOfNonzerosPerRow];
...
// C++ std::map is not threadsafe for writing
        ...
        DatatypeA * currentValuePointer = matrixValues[currentLocalRow]; // Pointer to current value in current row
        ...
        if (b!=0)      bv[currentLocalRow] = 26.0 - ((DatatypeB) (numberOfNonzerosInRow-1));
