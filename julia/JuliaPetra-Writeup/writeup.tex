...
The first type is the concrete type \snippet{MultiVector} which holds one or more dense vectors.
The second type is the \snippet{Operator} interface which is implemented by types that provide a
\(y \gets \alpha A(x) + \beta y\) operation, where \(x\) and \(y\) are vectors and \(A\) is the operator.
...
The main improvement to performance in JuliaPetra came from improving type stability.
Type stability is where the compiler can inference the types of all values.
When a function is type stable, the compiler is able to fully inline function calls, especially arithmetic and array operations.
...
The \snippet{code_warntype} function was a crucial tool in finding type instabilities.
%REVIEW citation?
%REVIEW mention that this is standard Julia advice?

One notable optimization came from using \snippet{Ptr} objects to pass views of arrays.
...
The heap allocation and garbage collection can be greatly reduced in some performance critical sections
However, this does lose access to higher level logic that Julia normally provides,
such as the array length being stored with the array its self.

Other, common and minor optimizations were also made.
Skipping unnecessary bounds checks was one such optimization.
%TODO get data to back this up
Another optimization was to ensure access array data in the order it is stored in memory.
Additionally, function calls in frequently used loops were replaced with specialized, inlined versions
that take into account the context of the calling method and avoid using generalized methods that may produce extra computations.
...
MPI used TCP for all inter-process communication.
